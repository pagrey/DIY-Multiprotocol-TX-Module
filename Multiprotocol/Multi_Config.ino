			...
			case 5:
				...
				for (uint8_t i = 0; multi_protocols[i].protocol != 0xFF; i++) {
					if (Protocol_Offset[active.rxid_block][3] == multi_protocols[i].protocol) {
						...
						memcpy(&packet_in[1],"Unknown",7);
						packet_in[8] = 0xB0+1;
						packet_in[9] = active.rxid_block;
						break;
					}
				}
				//packet_in[(strlen(multi_protocols[option].ProtoString))] = 0xB0+1;
				//packet_in[(strlen(multi_protocols[option].ProtoString)+1)] = active.rxid_block;
				break;
			case 6:
				//RX Number
